...
(function (root) {
    ...
    test("enlaces contextuales son naturales y tienen destinos locales reales", () => { const info = html.slice(html.indexOf('<section id="informacion"'), html.indexOf('<section id="metodologia"')); const methodology = html.slice(html.indexOf('<section id="metodologia"'), html.indexOf('<section id="medir-cintura"')); ok(info.indexOf('<a href="https://imoancy.com/herramientas/calculadora-porcentaje-masa-muscular/">Calculadora de Masa Muscular Esquelética</a>') >= 0); ok(methodology.indexOf('<a href="https://imoancy.com/herramientas/calculadora-imc/">Calculadora de IMC de Imoancy</a>') >= 0); ok(html.indexOf(">haz clic aquí<") < 0); const manager = $.NSFileManager.defaultManager; ok(manager.fileExistsAtPath(base + "/../calculadora-porcentaje-masa-muscular/index.html")); ok(manager.fileExistsAtPath(base + "/../calculadora-imc/index.html")); });
