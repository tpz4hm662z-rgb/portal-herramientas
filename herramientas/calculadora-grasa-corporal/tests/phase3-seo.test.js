...
(function (root) {
    ...
    test("enlaces contextuales son naturales, únicos, perceptibles y tienen destinos locales reales", () => { const info = html.slice(html.indexOf('<section id="informacion"'), html.indexOf('<section id="metodologia"')); const methodology = html.slice(html.indexOf('<section id="metodologia"'), html.indexOf('<section id="medir-cintura"')); const muscleUrl = "https://imoancy.com/herramientas/calculadora-porcentaje-masa-muscular/"; const bmiUrl = "https://imoancy.com/herramientas/calculadora-imc/"; ok(info.indexOf('<a class="enlace-contextual" href="' + muscleUrl + '">Calculadora de Masa Muscular Esquelética</a>') >= 0); ok(methodology.indexOf('<a class="enlace-contextual" href="' + bmiUrl + '">Calculadora de IMC</a>') >= 0); ok(count('class="enlace-contextual"') === 2); ok(count('href="' + muscleUrl + '"') === 2); ok(count('href="' + bmiUrl + '"') === 2); ok(css.indexOf(".enlace-contextual{") >= 0); ok(css.indexOf(".enlace-contextual:hover{") >= 0); ok(css.indexOf(".enlace-contextual:focus-visible{") >= 0); ok(html.indexOf(">haz clic aquí<") < 0); const manager = $.NSFileManager.defaultManager; ok(manager.fileExistsAtPath(base + "/../calculadora-porcentaje-masa-muscular/index.html")); ok(manager.fileExistsAtPath(base + "/../calculadora-imc/index.html")); });
